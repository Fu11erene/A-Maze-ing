...
- `3`（2進数 `0011`）→ 南・西の壁が開いている（北・東は閉じている）
- `A`（2進数 `1010`）→ 東・西の壁が閉じている（北・南は開いている）
...
最初は**棒倒し法**で迷路生成を実装していました。しかし盤面の中央に「42」の形（壁で閉じたセルの塊）を配置しようとしたとき、棒倒し法では生成できるマスの形に制限があり、42パターンと矛盾なく生成することができませんでした。そのため最終的に**穴掘り法**（コード上は `_carve`：壁で埋め尽くした盤面をランダムにDFSで掘り進める手法）で迷路を生成する方式に変更しました。

- `PERFECT=True` の場合は、穴掘り法で生成した完全迷路（ループなし、入口〜出口は必ず1本道）をそのまま出力します。
- `PERFECT=False`（デフォルト）の場合は、完全迷路を生成したあとに `_braid` で行き止まりのセルの壁を一部取り除いてループ（複数経路）を作り、Pac-Man的に遊べる盤面にしています。
- 入口から出口までの最短経路は、生成後にBFS（`_solve_with_bfs`）で求めています。

#### Why?

穴掘り法は「すべて壁で埋めた盤面から掘り進める」方式のため、あらかじめ42パターンの壁を固定してから掘り進めても、そのパターンを壊さずに迷路を生成できます。一方、棒倒し法は「棒（壁）を倒して迷路を作る」方式のため、利用可能なマスの形に制限があり、42パターンと衝突しやすいという課題がありました。この制約を避けるために穴掘り法を採用しました。

### Reuseability

`src/mazegen` は、迷路生成ロジックを `MazeGenerator` という単一のクラスにまとめ、他プロジェクトからそのまま `import` して再利用できるようにした独立モジュールです。`make build` で `mazegen-*.whl` としてビルドでき、`pip` でインストールして使うことができます。設定パース (`Config`)・迷路生成・最短経路探索(BFS)・ターミナル表示までがこの1パッケージに閉じているため、生成ロジックだけを取り出して別プロジェクトに組み込むことができます。

より詳しいAPIドキュメントは [`src/mazegen/README.md`](src/mazegen/README.md) にありますが、要点は以下の通りです。
...
`Config` にサイズ (`width`, `height`)、`seed`、完全迷路にするかどうか (`perfect`) などを渡すことで、生成される迷路をカスタマイズできます。
...
開発は大きく3つのフェーズに分けて進めました（詳細は [`PLAN.md`](PLAN.md) 参照）。

1. **Phase 0（環境構築・パーサー）**: `uv` による環境構築、Makefileの整備、設定ファイルのパース処理を先に固める。
2. **Phase 1（〜8月中）: 迷路生成**: 棒倒し法で迷路生成のベースラインを作成し、ターミナル出力・ファイル出力を実装。完了後にコードレビューとテストを行い、クラス構成を課題要件に合わせて整理。
3. **Phase 2（9月第1週）: 42パターン・迷路の解答**: 42パターンを含めた迷路生成、および入口から出口までの最短経路を求めるアルゴリズムを追加。
4. **Phase 3（〜9/11）: 仕上げ**: `PERFECT=False` 用の非完全迷路（ループあり）生成アルゴリズムを追加し、全体の検証、ターミナルUIの整備、解答経路のファイル出力を行い提出に備えた。
...
- GitHubのPull Requestを使って、機能ごとに小さくレビューしながら進められた。
- 迷路生成アルゴリズムのようなコアロジックはお互い手を動かして理解した上で実装したため、設計や例外ケースについて深い議論ができた。
- 棒倒し法の限界に早めに気づき、穴掘り法へ切り替えられた。

**改善できる点**

- テストは設定ファイルのパース部分 (`tests/`) にしか用意できておらず、迷路生成や探索ロジックの自動テストが手薄だった。
- MLXによるグラフィカル表示やアニメーションなどのボーナス機能には手が回らなかった。
- README等のドキュメント整備を最後のフェーズに回してしまったため、開発と並行してもう少し早くから書き進めればより手戻りが少なかった。
...
- コードレビューはGitHubのPull Requestベースで行いました。
