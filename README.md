...
<center><img src="assets/a-maze-ing.png" width="500"></center>

## Description

このプロジェクトは、42カリキュラムの一環として、設定ファイルをもとに迷路を生成・探索するプログラムをPythonで実装し、パッケージング・データバリデーション・アルゴリズム設計・テスト・チーム開発を学ぶことを目的としています。

### 課題の概要

設定ファイルを受け取り迷路を生成するユーザーインタラクティブなプログラムをPythonで実装しました。生成された迷路の情報は出力ファイルに書き込まれます。
コマンドライン引数で迷路生成の設定ファイルを渡すと迷路が生成され、ユーザーは迷路の再生成、スタートからゴールまでの道の表示、迷路の色の変更を行うことができます。この課題では、`uv`を用いたPythonプロジェクトの環境構築、パッケージのビルド方法、オブジェクト指向としたプログラミング手法、`pydantic`を用いたデータのバリデーション、迷路生成と迷路探索アルゴリズム、`pytest`を使ったテスト手法、`git`を用いたチーム開発を学びました。

### 言葉の定義

以下の解説をするにあたって、いくつか言葉を定義します。

- ブロック: ターミナル上で出力された際、`  `, `██`, `42`, `\\` のいずれかで表される1マスのこと。今回の実装では、このブロックを基本的な単位としてデータを2次元配列にして考えました。
- セル: 迷路生成アルゴリズムやMLXによるグラフィカルレンダリングにおいて扱われる1マスのこと。壁は独立したブロックではなく、セル間の接続の有無として表現される。課題のPDFでは1マスの表現をこの書き方で採用しています。

これらは、表示の方法の違いによって生まれるものであり、生成された迷路はどちらの方法でも表示することが可能です。

例えば、3x3のセルで表現される迷路は、ターミナル上では7x7のブロックとして表示されます。

### 設定ファイルのフォーマット

設定ファイルは、すべてセルを基準にした迷路の設定を行います。1行ごとに `KEY=VALUE` の形式で記述します。

| Key         | Description                                       | Example                |
| ----------- | ------------------------------------------------- | ---------------------- |
| WIDTH       | 迷路の横のセルの数                                        | `WIDTH=20`             |
| HEIGHT      | 迷路の縦のセルの数                                        | `HEIGHT=15`            |
...
3x3の迷路を、(0,0)をentry、(2,2)をexitとして生成した例
...
実行すると迷路を生成した上でターミナルに表示され、続けて `OUTPUT_FILE` で指定したファイルへ書き出されます。その後、以下の操作を番号で選択できます。
...
迷路はUnicodeのフルブロック文字（`█`）とANSIエスケープシーケンスを使ってターミナルに描画されます。

- 1ブロックは2文字幅（`██` / `42` / `\\\\` / 半角スペース2つ）で表現され、ターミナル上で正方形に近い比率になります。
- `board` は壁も1ブロックとして含む展開済みのグリッドになっており、壁のブロックがそのまま `██` として描画されます。通路（壁がない部分）は半角スペース2つです。
- 壁の色はデフォルトでは無色の `██` ですが、最短経路・入口・出口は壁の色から一定のオフセットをつけた色で描画されます。メニューの `3. Rotate the wall colours` を選ぶと、壁の色を含めたパレット全体（無色・赤・緑・青・黄の5パターン）が一斉にローテーションします。
- 盤面の中央に配置される「42」パターンは、`42` という文字列とバックスラッシュ `\\\\` で表現されます。盤面が小さく42パターンが収まらない場合は挿入されず、`WARN: Ommiting 42 pattern due to its size.` という警告が表示されます。
- 迷路を再生成・切り替えるたびに、ANSIエスケープ（`\033[H\033[J`）でカーソルを左上に戻して画面をクリアしてから盤面全体を描き直します。
...
#### 棒倒し方ではなく、穴掘り法による迷路生成

最初は**棒倒し法**で迷路生成を実装していました。しかし盤面の中央に「42」のパターンを配置しようとしたとき、棒倒し法では追加される壁のブロックの場所に制限があり、PERFECTにした際にループができてしまいました。そのため最終的に**穴掘り法**で迷路を生成する方式に変更しました。

- `PERFECT=True` の場合は、穴掘り法で生成した完全迷路（ループなし、入口〜出口は必ず1本道）をそのまま出力します。
- `PERFECT=False`（デフォルト）の場合は、完全迷路を生成したあとに `_braid` で行き止まりのブロックの壁を一部取り除いてループ（複数経路）を作り、Pac-Manのように遊べる盤面にしています。
...
また、課題で禁止されている「壁のない3x3のopen areaを作ってはいけない」という要件だけではなく、2x3や3x2のopen areaも作れない仕組みになっています。ターミナルでの出力上において、任意の2x2のブロックを選んだ際に最低1ブロックに壁があることが保証されるためです。

###### 迷路の例

以下の図は、ターミナルへの出力のフォーマットでの左下の部分を抜粋したものです。白くなっているブロックは、2つのアルゴリズムの今回の実装において必ず壁となっている箇所になっています。
...
BFS（Breadth-First Search / 幅優先探索）は、開始地点から近いノードの順に、キューを使って層（距離）ごとにグラフを探索していくアルゴリズムです。迷路の各セルをノード、壁のない隣接のセルへの通路をエッジと見なすと、迷路探索はグラフ探索の問題として扱えます。

`_solve_with_bfs` では、入口のセルをキューに入れてスタートし、上下左右の壁がないセルへ1セルずつ探索を広げながら、各セルに「どのセルから来たか」（`prev_cordinate`）を記録しています。出口のセルに到達した時点でキューを打ち切り、`prev_cordinate` を出口から入口まで逆順に辿ることで最短経路の座標集合と、進んだ方向（`N`/`E`/`S`/`W`）の文字列を復元しています。
...
- `maze_gen.board`: `list[list[FillStatus]]`。盤面全体をブロックの二次元配列として表現したもの。各ブロックは `mazegen.types.FillStatus`（`empty` / `wall` / `entry` / `exit` など）。
...
- pyproject.tomlでメタデータやツールの設定をまとめているため、プロジェクトに統一感をもたせることができた。
...
- Git hooksを使って、コミットメッセージにprefixがない場合にエラーを出すなど、Gitの扱いにもう少し一貫性を持たせても良かったと思った。
